...
public class TileEntityReactor extends WarpEnergyTE {
	...
	private boolean isShipInJumpgate(Jumpgate jg, StringBuilder reason) {
		...
		if (aabb.isVecInside(Vec3.createVectorHelper( // TODO: Unknown if will
				// work
	...
	public void messageToAllPlayersOnShip(String msg) {
		...
		System.out
				.println("" + (FMLCommonHandler.instance().getEffectiveSide().isClient() ? "Client" : "Server") + this + " messageToAllPlayersOnShip: " + msg);
		for (Object o : list) {
			if (o == null || !(o instanceof EntityPlayer)) {
				continue;
			}
	...
	private void summonPlayers() {
		...
		for (int i = 0; i < controller.players.size(); i++) {
			...
			EntityPlayerMP player = MinecraftServer.getServer().getConfigurationManager().func_152612_a(nick); // getPlayerForUsername
	...
	private void summonPlayersByChestCode() {
		...
		for (int i = 0; i < MinecraftServer.getServer().getConfigurationManager().playerEntityList.size(); i++) {
			...
			if (checkPlayerInventory(chest, player)) {
				System.out.println("" + this + " Summoning " + player.getDisplayName());
